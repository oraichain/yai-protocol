...
contract ERC20 is Initializable, Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20:1"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20:2"));
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        require(sender != address(0), "ERC20:3");
        require(recipient != address(0), "ERC20:4");

        _balances[sender] = _balances[sender].sub(amount, "ERC20:4");
    ...
    function _mint(address account, uint256 amount) internal {
        require(account != address(0), "ERC20:6");
    ...
    function _burn(address account, uint256 amount) internal {
        require(account != address(0), "ERC20:7");

        _balances[account] = _balances[account].sub(amount, "ERC20:8");
    ...
    function _approve(address owner, address spender, uint256 amount) internal {
        require(owner != address(0), "ERC20:9");
    ...
    function _burnFrom(address account, uint256 amount) internal {
        ...
        _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "ERC20:11"));
    }
...
contract ERC20Detailed is Initializable, IERC20 {
    string private _name = "aiUSDT_v2";
    string private _symbol = "aiUSDT_v2";
...
contract GovernableInit is Initializable {
    ...
    function onlyGovernance() internal {
        require(Storage(_storage()).isGovernance(msg.sender), "Gnv:1");
    }
    ...
    function initialize(address _store) internal initializer {
        _setStorage(_store);
    }
    ...
    function setStorage(address _store) public {
        require(Storage(_storage()).isGovernance(msg.sender), "Gvn:2");
        require(_store != address(0), "Gvn:3");
        _setStorage(_store);
    }
...
contract ControllableInit is GovernableInit {
    ...
    function initialize(address _storage) internal initializer {
...
contract VaultStorage is Initializable {
    ...
    function underlyingUnit() internal view returns (uint256) {
    ...
    function _sharePriceCheckpoint() public view returns (uint256) {
    ...
    function _allowSharePriceDecrease() public view returns (bool) {
    ...
    function vaultFractionToInvestNumerator() internal view returns (uint256) {
    ...
    function vaultFractionToInvestDenominator() internal view returns (uint256) {
    ...
    function strategyTimeLock() internal view returns (uint256) {
...
contract Vault is ERC20, ERC20Detailed, ControllableInit, VaultStorage {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    bool public isStrategyV1;
    ...
    function initializeVault(address _storage,
        ...
        require(_toInvestNumerator <= _toInvestDenominator, "V:1");
        require(_toInvestDenominator != 0, "V:2");
    ...
    function whenStrategyDefined() private {
        require(address(strategy()) != address(0), "V:3");
    }
    ...
    function defense() private {
        require(
            ...
            "V:4"  // make sure that it is not on our greyList.
        );
    }

    function setIsStrategyV1(bool value) external {
        onlyGovernance;
        isStrategyV1 = value;
    }
    ...
    */
    function doHardWork() public {
        ...
        whenStrategyDefined();
        if (futureStrategy() != address(0) && futureStrategy() != strategy() && strategyUpdateTime() <= block.timestamp) {
            if (IStrategy(futureStrategy()).underlying() == address(underlying()) && IStrategy(futureStrategy()).vault() == address(this)) {
                IERC20(underlying()).safeApprove(address(strategy()), 0);
                IStrategy(strategy()).withdrawAllToVault();
                _setStrategy(futureStrategy());
//                _setSharePriceCheckpoint(10 ** uint256(ERC20Detailed(address(underlying())).decimals()));
                IERC20(underlying()).safeApprove(address(strategy()), 0);
    ...
    */
    function underlyingBalanceWithInvestment() view public returns (uint256) {
        return address(strategy()) == address(0) ? underlyingBalanceInVault() : underlyingBalanceInVault().add(investedTokenAmount());
    }
    ...
    */
    function underlyingBalanceWithInvestmentForHolder(address holder) view external returns (uint256) {
        ...
        return balanceOf(holder).mul(Math.min(_sharePriceCheckpoint(), getPricePerFullShare())).div(underlyingUnit());
    }
    /**
    ...
    */
    function announceStrategyUpdate(address _strategy, uint256 when) external {
        onlyControllerOrGovernance();
        require(_strategy != address(0), "V:5");
        require(IStrategy(_strategy).underlying() == address(underlying()), "V:6");
        require(IStrategy(_strategy).vault() == address(this), "V:7");
        require(when > 0, "V:8");
    ...
    */
    ...
    function setStrategy(address _strategy) external {
        onlyControllerOrGovernance();
        require(strategy() == address(0), "V:9");
//        require(_strategy != address(0), "V:10");
        //        require(IStrategy(_strategy).underlying() == address(underlying()), "V:11");
        //        require(IStrategy(_strategy).vault() == address(this), "V:12");
...
//        _setSharePriceCheckpoint(10 ** uint256(ERC20Detailed(address(underlying())).decimals()));
    ...
    function setVaultFractionToInvest(uint256 numerator, uint256 denominator) external {
        onlyGovernance();
        require(denominator > 0, "V:13");
        require(numerator <= denominator, "V:14");
    ...
    */
    ...
    function withdraw(uint256 numberOfShares) external {
        require(totalSupply() > 0, "V:15");
        require(numberOfShares > 0, "V:16");
    ...
    function _deposit(uint256 amount, address sender, address beneficiary) internal {
        require(amount > 0, "V:17");
        require(beneficiary != address(0), "V:18");

        if (address(strategy()) != address(0)) {
            require(IStrategy(strategy()).depositArbCheck(), "V:19");
        }
        uint256 toMint = amount.mul(underlyingUnit()).div(
            Math.max(_sharePriceCheckpoint(), getPricePerFullShare())
        );
